...
pub trait PartitionedStream: std::fmt::Debug + Send {
    ...
    /// Removes the underlying stream for `stream_idx`, replacing it with an
    /// empty one. Used to hand the last non-exhausted stream through directly
    /// instead of merging it row-by-row.
...
impl FusedStreams {
    fn take_stream(&mut self, stream_idx: usize) -> Fuse<SendableRecordBatchStream> {
        ...
        mem::replace(&mut self.0[stream_idx], empty_stream.fuse())
    }
